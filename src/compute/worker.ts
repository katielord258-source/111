  ...
  try {
    switch (data.type) {
      ...
      case 'snapshot_request': {
        const { snapshot, series } = buildFullSnapshot(data.candles, data.config, data.activeFeatures, data.isClosed, isCrypto(data.symbolId));
...
function backtestInWorker(
  ...
  for (let i = warmup; i < candles.length - expiryBars - 1; i++) {
    ...
    const { snapshot } = buildFullSnapshot(slice, config, activeFeatures, true, isCrypto(symbolId));
