...
export interface PatternContext {
  ...
  // D1 (аудит "Исправление по воронке гейтов", 2026-09-2x): getSessionRegime()
  // размечает 'sydney'/'tokyo'/'closed' по календарю форекс-сессий (включая
  // выходные) — это корректно для форекса, но крипта торгуется 24/7 и не
  // "закрыта" по выходным/в азиатские часы в том смысле, в каком это верно
  // для FX. isAsiaOrClosed() ниже используется как ЖЁСТКИЙ гейт (return null)
  // почти во всех context-aware детекторах; для крипты это раньше отсекало
  // ~58% недели без рыночного основания (см. getSessionRegime-прогон в
  // промте). sessionAgnostic=true (проставляется в detectAllPatterns() по
  // isCrypto(symbolId) на стороне вызывающего) отключает именно этот жёсткий
  // гейт, но НЕ трогает sessionBoost() — мягкий множитель по реальной сессии
  // остаётся как есть и для крипты (внутридневная ликвидность у крипты и
  // правда выше в часы открытия традиционных рынков, см. промт, п. D1).
...
// D1: sessionAgnostic true (крипта, см. PatternContext.sessionAgnostic выше)
// полностью отключает этот гейт — жёсткого понятия "закрыт"/"азиатская
// сессия" для 24/7-инструмента нет. Для форекса (sessionAgnostic не
// передан/false) поведение не меняется.
export function isAsiaOrClosed(session: SessionRegime, sessionAgnostic?: boolean): boolean {
  if (sessionAgnostic) return false;
