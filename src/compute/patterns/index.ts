...
export function detectAllPatterns(
  ...
  // D1: true для крипто-символов (isCrypto(symbolId) на стороне вызывающего —
  // full-snapshot.ts/horizon-audit.ts) — отключает жёсткий сессионный гейт
  // isAsiaOrClosed() во всех context-aware детекторах ниже, см.
  // PatternContext.sessionAgnostic в pattern-context.ts. По умолчанию false —
  // поведение для форекса и для существующих вызовов без этого аргумента
  // (тесты и т.п.) не меняется.
  sessionAgnostic: boolean = false,
): PatternResult[] {
