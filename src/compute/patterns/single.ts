...
export function detectHammer(ctx: PatternContext): PatternResult | null {
  ...
  diagCount(`hammer:htf-seen-${htfClassOf(ha)}`); // D3 п.6 — только измерение
...
export function detectShootingStar(ctx: PatternContext): PatternResult | null {
  ...
  // D2 (промт "Исправление по воронке гейтов", п.5 "Асимметрия hammer-
  // семейства"): у shooting-star не было RSI-гейта, хотя геометрически и по
  // трендовому контексту (разворот ПОСЛЕ восходящего импульса, направление
  // sell) это тот же случай, что у hanging-man — а там гейт есть (rsi<60 →
  // отказ). Раньше это было единственной прямой асимметрией из четырёх
  // членов семейства (hammer/inverted-hammer уже требуют rsi>40 в свою
  // сторону), и именно поэтому у shooting-star после отключения сессионного
  // гейта (D1) occurrences росли иначе, чем у остальных трёх паттернов.
  // Зеркалим hanging-man дословно: тот же порог 60, то же направление
  // отказа, тот же rsiFactor ниже и тот же потолок base/confidence (0.45),
  // а не наоборот — снятие гейтов у hammer/inverted-hammer/hanging-man
  // отменило бы уже проверенное на аудите поведение трёх детекторов ради
  // одного.
  if (indicators && indicators.rsi != null && indicators.rsi < 60) return null;
  gate('shooting-star:03-rsi');
  ...
  // D2: потолок base выровнен с hammer/hanging-man (0.25+0.15+0.05=0.45,
  // было 0.30+0.10+0.10=0.50) — иначе тест зеркальной симметрии не
  // проходит даже после добавления RSI-гейта выше: у shooting-star
  // оставался бы более высокий потолок confidence и другой порог входа
  // (0.50 против 0.45), чем у геометрически идентичного hanging-man.
  const base = clamp01(
  ...
  const rsi = indicators?.rsi ?? null;
  // Зеркало hanging-man rsiFactor (rsi>=70?1.10:rsi>=50?1.0:0.8) — тот же
  // sell-паттерн, тот же RSI-гейт(60) выше, поэтому и градация усиления
  // должна быть той же, а не собственной шкалой.
  const rsiFactor = rsi == null ? 1.0 : rsi >= 70 ? 1.10 : rsi >= 50 ? 1.0 : 0.8;

  const factors: string[] = [];
  const sb = sessionBoost(session);
  const ha = htfAlignment(htfStructure, 'sell');
  diagCount(`shooting-star:htf-seen-${htfClassOf(ha)}`); // D3 п.6 — только измерение
  const vf = volumeFactor(volumeRatio);
...
export function detectInvertedHammer(ctx: PatternContext): PatternResult | null {
  ...
  const sb = sessionBoost(session);
  const ha = htfAlignment(htfStructure, 'buy');
  diagCount(`inverted-hammer:htf-seen-${htfClassOf(ha)}`); // D3 п.6 — только измерение
  const vf = volumeFactor(volumeRatio);
  const obBonus = obFvgConfluenceBonus(smartMoney, patternCandle, 'buy', indicators?.atr ?? null, intervalSeconds(candles));
...
export function detectHangingMan(ctx: PatternContext): PatternResult | null {
  ...
  const sb = sessionBoost(session);
  const ha = htfAlignment(htfStructure, 'sell');
  diagCount(`hanging-man:htf-seen-${htfClassOf(ha)}`); // D3 п.6 — только измерение
  const vf = volumeFactor(volumeRatio);
  const obBonus = obFvgConfluenceBonus(smartMoney, patternCandle, 'sell', indicators?.atr ?? null, intervalSeconds(candles));
