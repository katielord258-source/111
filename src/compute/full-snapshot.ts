...
export function buildFullSnapshot(
  ...
  // D1: true для крипто-инструментов — см. PatternContext.sessionAgnostic
  // в pattern-context.ts. Вызывающая сторона (analysisEngine.ts, worker.ts)
  // определяет это через isCrypto(symbolId) из '@/data/symbols'.
  ...
  // direction-prediction.ts, signal-filters.ts, patterns/index.ts).
