...
import { findSymbol, TIMEFRAME_SECONDS, isCrypto } from '@/data/symbols';
...
export const useTickStore = create<TickState>((set, get) => ({
  ...
  start: async (symbolId: string, timeframe: Timeframe) => {
    ...
    try {
      ...
      const { snapshot: fullSnap, series: fullSeries } = buildFullSnapshot(compacted, settings.indicators, features, true, isCrypto(symbolId));
      set({
        candles: compacted,
