...
export const useDemoAccountStore = create<DemoAccountState>()(
  persist(
    (set, get) => ({
      ...
      // BUGFIX (промт "Исправление по воронке гейтов", Фаза 0): было true/true
      // по умолчанию — новый/сброшенный демо-счёт сразу торговал вживую
      // непроверенными сигналами (0 valid-паттернов на аудите на момент
      // фикса) да ещё с мартингейлом, усиливающим серию убытков при нулевом
      // или отрицательном ожидании. У существующих пользователей с уже
      // сохранённым состоянием этот дефолт ничего не меняет (persist берёт
      // их собственное значение) — влияет только на новые/сброшенные счета.
